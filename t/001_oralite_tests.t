...
my $latin1 = DBD::OraLite::_latin1_symbol();
is ( functest($dbh,"convert('á','$latin1')"), "á", "convert() (2-arg)");
is ( functest($dbh,"convert('á','$latin1','UTF-8')"), "Ã¡", "convert() (3-arg)");
is ( functest($dbh,"decode('UGdMaXRl', 'base64')"), "OraLite", "decode()");
is ( functest($dbh,"encode('OraLite','base64')"), "UGdMaXRl", "encode()");
...
is ( functest($dbh,"position('Li' in 'OraLite')"), 3, "position()");
...
EOF
  ;
	...
	DBD::OraLite::_register_stored_functions($dbh);
	return 1;
}
