package DBD::OraLite::MirrorOraToSQLite;
use strict;
use DBI;
use Storable;
use File::Copy;
...
DBD::OraLite::MirrorOraToSQLite - Mirror tables from Oracle to SQLite

=head1 SUMMARY

 use DBD::OraLite::MirrorOraToSQLite qw(pg_to_sqlite);
 pg_to_sqlite(
...
Oracle database to a SQLite file. The module has only be tested
...
for use with DBD::OraLite only, since these functions are put into the
