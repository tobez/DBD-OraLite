...
sub driver {
	...
	($drh) = DBI::_new_drh ($class, {
		'Name' => 'OraLite',
		'Version' => $VERSION,
		'Attribution' => 'DBD::OraLite by Anton Berezin and Jonas B. Nielsen',
	});
	return $drh;
}
...
sub _register_builtin_functions {
	...
	$dbh->func( "avg", 1, 'DBD::OraLite::Aggregate::avg', "create_aggregate" );
}
...
sub connect {
    ...
    DBD::OraLite::_register_builtin_functions($real_dbh);
	...
	DBD::OraLite::_register_stored_functions($handle);
	DBD::OraLite::setDbh($handle);
	return $handle;
}
...
sub begin_work       { DBD::OraLite::setTransaction(1); DBD::OraLite::setTime(); shift->{D}->begin_work(@_); }
sub commit           { DBD::OraLite::setTransaction(0); DBD::OraLite::setTime(); shift->{D}->commit(@_); }
sub rollback         { DBD::OraLite::setTransaction(0); DBD::OraLite::setTime(); shift->{D}->rollback(@_); }


sub prepare {
	my ($dbh,$statement,$attr) = @_;
	my $filtered = DBD::OraLite::Filter::filter_sql($dbh,$statement,$attr);
...
sub filter_sql {
	...
	DBD::OraLite::setTime() unless DBD::OraLite::getTransaction();
...
package DBD::OraLite::Aggregate::avg;
...
DBD::OraLite::MirrorOraToSQLite companion module.
...
in PostgreSQL and OraLite (e.g. with the function name
'full_price_descr'):
...
=head2 DBD::OraLite::MirrorPgToSQLite

The companion module, DBD::OraLite::MirrorPgToSQLite, may be of use in
